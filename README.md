#快速入门
##①定义实体，实体中的字段基本和数据库字段保持一致即可，也就是说，自行定义的实体，只要有需要操作的数据库的字段有对应即可，字段可多可少，也可以没有，只要类名能匹配表名就行。
##匹配规则，可以强行匹配，如数据库字段是sys_user_name 那么对象字段可以是sys_user_name 同时也可以是sysUserName（驼峰命名法，建议这样子）
```java
