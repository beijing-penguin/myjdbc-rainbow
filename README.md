#快速入门
##①定义实体，首先类名得对应表名（只在对实体操作时强制类名和表名保持一致的要求），实体中的字段基本和数据库字段保持一致即可，实体中只要有数据库的字段有对应即可，字段可多可少。实体对象的字段和对象名遵循驼峰命名法。例如数据库表名user_table_system，那么类名则可以是User_table_system，也可以是UserTableSystem
